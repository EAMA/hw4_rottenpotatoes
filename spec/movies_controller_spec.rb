...
describe Movie do
  ...
  it "call new created method find same by director" do
    ...
    @fake_movie.stub(:director).and_return("Me")
    Movie.find_same_by_director(@fake_movie.director)
  end
  end
...
describe MoviesController do
  describe 'Restful route for Find Similar Movies' do
    ...
    it 'creates movie' do
    post :create, {:movie_title => @fake_movie.title,
                   :movie_rating => @fake_movie.rating,
                   :movie_director => @fake_movie.director,
                   :movie_release_date => @fake_movie.release_date}
  ...
  describe 'Controller method to receive the click on "Find with same director"' do
    ...
    Movie.should_receive(:find_same_by_director).and_return(@fake_movies)
    end
  ...
  describe 'cant find movies and return to home page' do
    ...
    Movie.should_receive(:find_by_id).and_return (@fake_movie)
    Movie.should_not_receive(:find_same_by_director).and_return(@fake_movie)
    get :findmovieswithsamedirector, {:id => '3'}
    response.should redirect_to movies_path
    end
